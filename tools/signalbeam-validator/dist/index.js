...
server.tool("detect_changes", "Detect which areas of the codebase changed relative to origin/main. Returns flags for backend, frontend, infra, endpoints, entities, and events.", {}, async () => {
    ...
    const flags = {
        ...
        hasInfra: lines.some((f) => f.startsWith("infra/") ||
            f.startsWith("deploy/") ||
...
server.tool("validate_layers", "Check hexagonal architecture layer violations for a microservice. Verifies Domain has no deps on Infrastructure/Host, Application does not reference Host.", { service: z.enum(SERVICES).describe("Service name, e.g. DeviceManager") }, async ({ service }) => {
    const violations = [];
    ...
    if (domainCsproj) {
        ...
        for (const ref of refs) {
            if (ref.includes("Infrastructure") || ref.includes("Host")) {
                violations.push(`Domain references ${path.basename(ref)} — Domain must have zero deps on Infrastructure/Host`);
            }
        }
    }
    ...
    if (appCsproj) {
        ...
        for (const ref of refs) {
            if (ref.includes("Host")) {
                violations.push(`Application references ${path.basename(ref)} — Application must not reference Host`);
            }
        }
    }
    ...
    if (infraCsproj) {
        ...
        for (const ref of refs) {
            if (ref.includes("Host")) {
                violations.push(`Infrastructure references ${path.basename(ref)} — Infrastructure must not reference Host`);
            }
        }
    }
    return {
...
server.tool("check_pending_migrations", "Check if a microservice has EF Core model changes without a corresponding migration.", { service: z.enum(SERVICES).describe("Service name, e.g. DeviceManager") }, async ({ service }) => {
    const servicePaths = findServicePaths(service);
    ...
    try {
        const output = run(`dotnet ef migrations has-pending-model-changes --project "${servicePaths.infrastructure}" --startup-project "${servicePaths.host}" 2>&1`);
        // Exit code 0 = no pending changes
        return {
            content: [
                {
                    type: "text",
                    text: JSON.stringify({ service, pending: false, output: output.trim() }, null, 2),
                },
            ],
        };
    }
        ...
        return {
            content: [
                {
                    type: "text",
                    text: JSON.stringify({ service, pending: true, output: msg }, null, 2),
                },
            ],
        };
    }
...
server.tool("check_result_pattern", "Check if a C# handler file uses the Result pattern correctly — flags thrown business exceptions that should return Result<T> instead.", { filePath: z.string().describe("Absolute or repo-relative path to a .cs handler file") }, async ({ filePath }) => {
    const absPath = path.resolve(path.isAbsolute(filePath) ? filePath : path.join(REPO_ROOT, filePath));
    ...
    const catchesGenericException = /catch\s*\(\s*Exception\s/.test(content) &&
        !/throw;/.test(content);
    const issues = [];
    if (throwMatches.length > 0) {
        issues.push(`Throws ${throwMatches.length} business exception(s): ${throwMatches.join(", ")} — use Result<T> instead`);
    }
    if (hasEmptyCatch) {
        issues.push("Empty catch block detected — handle or propagate the error");
    }
    if (catchesGenericException) {
        issues.push("Catches generic Exception without re-throw — catch specific exceptions or convert to Result");
    }
    return {
        content: [
            {
...
server.tool("validate_all_layers", "Check hexagonal architecture layer violations across all microservices in one call.", {}, async () => {
    const results = {};
...
server.tool("check_all_migrations", "Check all microservices for pending EF Core migrations in one call.", {}, async () => {
    const results = {};
    for (const service of SERVICES) {
        const servicePaths = findServicePaths(service);
        if (!servicePaths?.infrastructure || !servicePaths?.host) {
        ...
        try {
            run(`dotnet ef migrations has-pending-model-changes --project "${servicePaths.infrastructure}" --startup-project "${servicePaths.host}" 2>&1`);
            // Exit code 0 = no pending changes
...
function findServicePaths(service) {
    ...
    for (const prefix of prefixes) {
        ...
        for (const entry of entries) {
            ...
            if (name.endsWith(".Host") || name === "Host")
                paths.host = csproj;
        }
...
function extractProjectRefs(csprojContent) {
    const matches = csprojContent.match(/Include="([^"]*\.csproj)"/g);
    if (!matches)
        return [];
