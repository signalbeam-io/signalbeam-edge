...
// The web origin the OIDC app redirects back to, and the OIDC issuer/authority the
// SPA + backend use. Defaults target local dev; override for the deployed env
// (e.g. WEB_BASE_URL=https://<swa-host>, OIDC_AUTHORITY=https://<zitadel-fqdn>).
var webBaseUrl = (Environment.GetEnvironmentVariable("WEB_BASE_URL") ?? "http://localhost:5173").TrimEnd('/');
var oidcAuthority = Environment.GetEnvironmentVariable("OIDC_AUTHORITY") ?? "http://localhost:8080";
var requireHttpsMetadata = (Environment.GetEnvironmentVariable("BACKEND_REQUIRE_HTTPS") ?? "false")
    .Equals("true", StringComparison.OrdinalIgnoreCase);
// Zitadel only allows non-HTTPS redirect URIs when the app is in dev mode.
var devMode = webBaseUrl.StartsWith("http://", StringComparison.OrdinalIgnoreCase);
...
// Step 1: Obtain the PAT. Locally Zitadel writes it to a file on first init; for a
// deployed Zitadel (no shared volume) the operator supplies it directly via the
// ZITADEL_PAT env var (create a service user + PAT in the Zitadel console).
var patToken = Environment.GetEnvironmentVariable("ZITADEL_PAT");
if (string.IsNullOrWhiteSpace(patToken))
{
    patToken = await ReadPatWithRetryAsync(patFilePath);
}
else
{
    Console.WriteLine("✅ Using PAT from ZITADEL_PAT environment variable");
}
...
else
{
    ...
    {
        name = "SignalBeam Web",
        redirectUris = new[] { $"{webBaseUrl}/callback", $"{webBaseUrl}/silent-renew" },
        postLogoutRedirectUris = new[] { webBaseUrl },
